...
public class PrinterActivity extends FragmentActivity implements BluetoothHelperEventListener {
	...
	public void deviceSelected(BluetoothDevice item) {
		if (isTablet()) {
			PrinterActionListFragment actionFragment = (PrinterActionListFragment) getSupportFragmentManager().findFragmentByTag(
					"detail");
			...
			getSupportFragmentManager().beginTransaction().replace(R.id.main, actionFragment, "main").addToBackStack(null)
					.commit();
		}
